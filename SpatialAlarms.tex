...
%\Procedure{ClientInitByServer}{}
	...
	\While {$\left\vert{P}\right\vert < 1$ }{
	    ...
	    $P \gets \textsc{CheckNewPOI}(q, r_{kp}, q, 0) $
	}
...
%\Procedure{ConfigAlarm}{}
    ...
    \ForEach{$p_i \in P$}{
    		$p_i.u \gets \textsc{GetPoiRange}(p_i)$
    		$r_{safe} \gets min( r_{safe}, p_i.d_O - p_i.u )$ \;
    		$u_{max} \gets max(u_{max}, p_i.u)$
    }
     $r_{rel} \gets (r_{ko} - u_{max})$ \;
    
    \ForEach{$p_i \in P$}{
    		\If{$p_i.d_O \leqslant p_i.u$}{
    			$\textsc{AlarmUser}(p_i)$\;
    		}
...
%\Procedure{UpdateAlarm}{}
    ...
    \If{$d_q > r_{rel}$}{
    		$A \gets \textsc{ClientInitByServer}(q, d_q)$ \;
