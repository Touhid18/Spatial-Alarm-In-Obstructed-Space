...
\title{Spatial Alarms In The Obstructed Space\titlenote{(Produces the permission block, and
...
The widespread use of smart phones has led to the proliferation of location based services. %According to many, the next step in location based services is Spatial Alarms. Many believe this particular feature is going to dominate the future mobile-phone computing systems./%
...
%If we start to evaluate spatial alarms as soon as the user is on the move even if the user is far away from her desired location our efforts will be futile.
...
\caption{\textsc{UpdateClient}$(q', A)$}
    ...
    \Input{$q', A$}
	 ...
	 \For{$i \gets 1$ to $\left\vert{P}\right\vert$}{
		\If{$dist_O(q', p_i, V_G) > u_i$}{
			$\textsc{AlarmUser}(p_i)$\;
		}
	}
	 \If {$dist_E(q, q') > k_{min}$ }{
		\textsc{InitByServer}$(q', 100, 100)$
	}
\label{UpdateClient}
%\end{algorithmic}
\end{algorithm}
...
%\begin{algorithmic}[2]
%\Procedure{ClientInitByServer}{}

    \SetKwInOut{Input}{Input}
    \SetKwInOut{Output}{Output}
    \Input{Query point $q$, increment delta $r_d$}
    \Output{The answer set, $A \gets \left\{ {r_{kp}, r_{ko}, P, O}\right\}$}
...
%\Procedure{UpdateAlarm}{}
    ...
    \If{$q_d > r_{rel}$}{
    		$A \gets \textsc{ClientInitByServer}(q, q_d)$ \;
    		\textsc{ConfigAlarm}($q, A$) \;
    }
